...
class BadgeCase_Scene
  ...
  def drawCasePage
    ...
    for i in 0...@badges.length
      @sprites["badge#{i}"].setBitmap("Graphics/Pictures/Badgecase/Badges/#{@badges[i].id}") if (($PokemonGlobal.badges.has?(@badges[i].id)) || BADGE_BLACK)
      (($PokemonGlobal.badges.has? (@badges[i].id)) || !BADGE_BLACK) ? @sprites["badge#{i}"].tone = Tone.new(0,0,0,0) : @sprites["badge#{i}"].tone = Tone.new(-255,-255,-255,-255)
    end
  ...
  def drawBadgePage
    ...
    @sprites["badge"].visible = $PokemonGlobal.badges.has?(@badges[@badgeindex].id)
    @sprites["leadersprite"].setBitmap("Graphics/Trainers/#{@badges[@badgeindex].leadersprite}")
    @sprites["leadersprite"].visible = $PokemonGlobal.badges.has?(@badges[@badgeindex].id)
    ...
    @sprites["acepokemon"].setBitmap("Graphics/Pokemon/Front/#{$PokemonGlobal.badges.has?(@badges[@badgeindex].id)? @badges[@badgeindex].acepokemon.upcase : "000"}")
    textpos = [
      [_INTL("#{($PokemonGlobal.badges.has?(@badges[@badgeindex].id) || BADGE_LOCATION_ALWAYS)? @badges[@badgeindex].name : "???"}"), 312, 214, 0, base, shadow],
      [_INTL("Leader: #{$PokemonGlobal.badges.has?(@badges[@badgeindex].id)? @badges[@badgeindex].leadername : "???"}"), 20, 254, 0, base, shadow]
    ]
    ypos = 284
    textpos.push([_INTL("Location: #{($PokemonGlobal.badges.has?(@badges[@badgeindex].id) || BADGE_LOCATION_ALWAYS)? @badges[@badgeindex].location : "???"}"), 20, ypos, 0, base, shadow]) && ypos += 30 if BADGE_SHOW_LOCATION
    textpos.push([_INTL("Main Type: #{$PokemonGlobal.badges.has?(@badges[@badgeindex].id)? @badges[@badgeindex].type : "???"}"), 20, ypos, 0, base, shadow]) && ypos += 30 if BADGE_SHOW_TYPE
    textpos.push([_INTL("Ace Pokemon: #{$PokemonGlobal.badges.has?(@badges[@badgeindex].id)? @badges[@badgeindex].acepokemon.capitalize : "???"}"), 20, ypos, 0, base, shadow]) && ypos += 30 if BADGE_SHOW_ACE_POKEMON
    pbDrawTextPositions(overlay,textpos)
    @sprites["pokemonoverlay"].visible = BADGE_SHOW_ACE_POKEMON
    @sprites["acepokemon"].visible = BADGE_SHOW_ACE_POKEMON
    @sprites["background"].setBitmap("Graphics/Pictures/Badgecase/Backgrounds/#{@badges[@badgeindex].type.downcase}") if $PokemonGlobal.badges.has?(@badges[@badgeindex].id) && BADGE_SHOW_TYPE && BADGE_MATCH_BACKGROUND
